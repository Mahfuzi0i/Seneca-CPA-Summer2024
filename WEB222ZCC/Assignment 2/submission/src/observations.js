...
function speciesCoordinates(data) {
  ...
  for (i = 0; i < results.length; i++) {
    ...
    const formattedString = `"${species}" observed at coordinates (${location})`;
...
function observationsByQualityGrade(data, qualityGrade) {
  ...
  const filteredObservations = []; // Create an empty array to hold the filtered observations

  // Check if qualityGrade is one of the valid values (case-insensitive)
  if (qualityGrade === undefined || (typeof qualityGrade !== 'string' && qualityGrade !== null)) {
    throw new Error(`Invalid quality grade: ${qualityGrade}`);
  }

  const lowerCaseGrade = qualityGrade === null ? null : qualityGrade.toLowerCase();

  if (!validQualityGrades.includes(lowerCaseGrade)) {
    throw new Error(`Invalid quality grade: ${qualityGrade}`);
  }

  // Loop through the results and add matching observations to the array
  data.results.forEach((observation) => {
    if (observation.quality_grade === lowerCaseGrade) {
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  if (qualityGrades.length < 1) {
    throw new Error('At least one qualityGrade value must be provided');
  }

  qualityGrades.forEach((grade) => {
    const lowerCaseGrade = grade === null ? null : grade.toLowerCase();
    const observations = observationsByQualityGrade(data, lowerCaseGrade);
...
function transformObservation(original) {
  ...
  if (original.conservation_status && original.conservation_status.status_name) {
...
function transformObservations(data) {
  ...
  data.results.forEach((observation) => {
    transformedArray.push(transformObservation(observation));
  });
...
function getObservationsByLocation(data, options = {}) {
  ...
  if (!Object.keys(options).length || (!options.lat && !options.lng)) {
    return data.results;
  }

  if (typeof lat === 'number' && typeof lng === 'number') {
    return data.results.filter((observation) => {
      const [obsLat, obsLng] = observation.location.split(',').map(parseFloat);
  ...
  if (
    ...
    return data.results.filter((observation) => {
      const [obsLat, obsLng] = observation.location.split(',').map(parseFloat);
      return obsLat >= lat.min && obsLat <= lat.max && obsLng >= lng.min && obsLng <= lng.max;
    });
  }
