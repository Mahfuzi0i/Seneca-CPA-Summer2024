...
function toCamelCase(name, uppercase = false) {
  // splitting the strings by dashes and filtering out the empty strings
  let parts = name.split('-').filter((part) => part.length > 0);

  // Capitalize the first letter of each part except for the first one if lowercase
  for (let i = 1; i < parts.length; i++) {
    parts[i] = parts[i][0].toUpperCase() + parts[i].slice(1).toLowerCase();
  }

  // join them back together
  let camelCaseString = parts.join('');

  // if uppercase is true, capitalize the first letter of the resulting string
  if (uppercase && camelCaseString.length > 0) {
    camelCaseString = camelCaseString[0].toUpperCase() + camelCaseString.slice(1);
  }
...
function createLinkTag(rel, href) {
  ...
  // Regular expression to test for special characters (space, tab, newline, line feed)
  const specialCharsRegex = /[\s\t\n\r]/;

  // Determine if quotes are needed for the rel attribute
  let relAttribute = specialCharsRegex.test(rel) ? `rel="${rel}"` : `rel=${rel}`;

  // Determine if quotes are needed for the href attribute
  let hrefAttribute = specialCharsRegex.test(href) ? `href="${href}"` : `href=${href}`;

  // Create the link tag string
  let linkTag = `<link ${relAttribute} ${hrefAttribute}>`;
...
function parseDateString(value) {
  ...
  month = parseInt(month, 10) - 1; // Month is zero-based in JavaScript Date
  day = parseInt(day, 10);
  ...
  if (month < 0 || month > 11) {
    throw new Error('Month must be a 2-digit number between 01 and 12.');
  }

  if (day < 1 || day > 31) {
    throw new Error('Day must be a 2-digit number between 01 and 31.');
  }
  ...
  date.setMonth(month);
  date.setDate(day);
  date.setHours(0, 0, 0, 0); // Ensure the time part is set to midnight
...
// An extension of problem 3 is in use "function parseDateString(value)"
function toDateString(value, format) {
  try {
    ...
    if (!validFormats.includes(format)) {
      throw new Error(
        'Invalid format string, expected "YYYY/MM/DD", "DD/MM/YYYY", or "MM/DD/YYYY.'
      );
    }
...
function normalizeTime(value) {
  ...
  const militaryRegex = /^(\d{1,2}):(\d{2}):(\d{2})$/;

  let hours, minutes, seconds, period;

  if (ampmRegex.test(value)) {
  ...
  } else if (militaryRegex.test(value)) {
    const match = value.match(militaryRegex);
    hours = parseInt(match[1], 10);
    minutes = parseInt(match[2], 10);
    seconds = parseInt(match[3], 10);
...
function formatTimes(...values) {
  const formattedTimes = values
    .map(normalizeTime) // Parse each time string
    .filter((time) => time !== null); // Filter out invalid times

  return `[${formattedTimes.join(', ')}]`; // Join valid times in the specified format
}
...
function mimeTypeFromFilename(filename) {
  ...
  // Using the switch statement to determin the mime type based on extension
  switch (extension) {
    case 'txt':
      return 'text/plain';
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  // Validate resource type and resource ID
